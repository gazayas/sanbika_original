...
function has_chordz(ary) {
  for (var i = 0; i < ary.length; i++) {
    if (WHITE_SPACE_REGEXP.test(ary[i]) || ary[i] == "") {// String is empty, go to next value
    } else if (!CHORD_REGEXP.test(ary[i]) || /Chorus|CHORUS|Bridge|BRIDGE/.test(str_ary[i])) {
      return false;
    } else {
      return true;
    }
...
$(document).ready(function () {
  ...
  for (var z = 0; z < song_bodies.length; z++) {
    ...
    for (var i = 0; i < song_body.length; i++) {
      ...
      var has_chords = has_chordz(str_ary); // If the array of strings consists of chords (as opposed to lyrics),
...
/******/ });
//# sourceMappingURL=initialize_song-97d881cd389ddc6b58a8.js.map
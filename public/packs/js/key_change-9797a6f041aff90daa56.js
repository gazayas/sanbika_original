...
// so the key change logic handles only the note itself.
...
function key_change(old_key, slash_chords) {
  ...
  return function (new_key) {
    ...
    var info = find_key_change_difference(old_key, new_key);
    var key_up = info[0];
    var difference = info[1];
    ...
    for (i = 0; i < chord_strings.length; i++) {
      ...
      if (/\//.test(chord_strings[i])) {
        ...
        var results = isolate_addition_from(chord_strings[i]);
...
/******/ });
//# sourceMappingURL=key_change-9797a6f041aff90daa56.js.map
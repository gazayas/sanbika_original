...
/* WEBPACK VAR INJECTION */(function($) {/* The following code is used primarily in songs#show */
...
function has_chords(ary) {
  for (var i = 0; i < ary.length; i++) {
    if (WHITE_SPACE_REGEXP.test(ary[i]) || ary[i] == "") {// String is empty, go to next value
...
// (as opposed to white space) at the beginning of the string.


function wrap_chords_with_span(ary) {
  for (i = 0; i < ary.length; i++) {
    if (/^\w/.test(ary[i])) {
      ary[i] = '<span class="chord" name="' + replace_marks(ary[i]) + '">' + replace_marks(ary[i]) + '</span>';
    }
  }
...
$(document).ready(function () {
  ...
  for (var i = 0; i < song_bodies.length; i++) {
    var song_body = song_bodies[i].innerHTML.split(/\r*\n/); // for(var j = 0; j < song_body.length; j++) {

    $.each(song_body, function (j) {
      var str_ary = song_body[j].split(WHITE_SPACE_REGEXP);

      if (has_chords(str_ary)) {
        str_ary = wrap_chords_with_span(str_ary);
      }

      song_body[j] = str_ary.join("");
    }); // }
    // Put the song body together again by joining the array of strings

    song_body = song_body.join("<br>"); // TODO: Look into what's going on here and refactor this next line
...
function toggle_visibility() {
  ...
  for (var i = 0; i < chords.length; i++) {
    chords[i].style.visibility = visibility;
  }
}

$(function () {
  return $('.visibility_checkbox').on('click', function () {
...
/******/ });
//# sourceMappingURL=initialize_song-b459d2b284b55dbc7b95.js.map
...
/* WEBPACK VAR INJECTION */(function($) {var SHARP_NOTES = ["A", "A♯", "B", "C", "C♯", "D", "D♯", "E", "F", "F♯", "G", "G♯"];
var FLAT_NOTES = ["A", "B♭", "B", "C", "D♭", "D", "E♭", "E", "F", "G♭", "G", "A♭"];
var ADDITIONS = ["add2", "add9", "dim7", "dim", "sus4", "sus", "maj7", "maj", "m7", "m", "aug", "2", "6", "7", "9", "11"];
...
// to declare the proper strings to finish off the key change.
...
function key_change(old_key, slash_chords) {
  ...
  var song_body = arguments.length > 3 ? arguments[3] : undefined;
  return function (new_key) {
    ...
    var chord_strings = [];
    var new_chords = []; // If an array of slash chords (i.e. ["G", "B"]) has been
    // passed to the function, use it as is.
    // Otherwise, pull the chords from their
    // span elements and push them to chord_strings.
    ...
    var results = find_key_change_difference(old_key, new_key);
    var key_up = results[0];
    var difference = results[1];

    for (i = 0; i < chord_strings.length; i++) {
      // If the chords are slash chords, they are split
      // here and run into another iteration of key_change.
      if (/\//.test(chord_strings[i])) {
        ...
        var new_position = get_new_chord_position(chord, difference, key_up);
        /*
...
if (set_list_songs.length >= 1) {
  $('.song_body').each(function () {
    ...
    var new_key = keys[1];
    $(function () {
      return $('.set_list_song').on('load', key_change(original_key, null, new_key, _this));
    });
  });
} else if (set_list_songs.length == 0) {
  var song_body = document.getElementsByClassName("song_body")[0];
  $(function () {
    return $('#key_box').on('change', function () {
      return key_change($('#key_box').attr('class'), null, null, song_body);
    });
  });
}
...
/******/ });
//# sourceMappingURL=key_change-0f6763c6541ac50ff340.js.map
...
var WHITE_SPACE_REGEXP = /(\s+|　+)/; // TODO: sharp_or_flat_is_present_in(str)
...
$(document).ready(function () {
  ...
  for (var z = 0; z < song_bodies.length; z++) {
    ...
    for (var i = 0; i < song_body.length; i++) {
      var has_chords; // The follwing Regular Expression has a Japanese white-space character in it.
      // TODO: For clarity, it might be better to simply change it a hexidecimal value.

      var str_ary = song_body[i].split(WHITE_SPACE_REGEXP); // TODO: Refactor this expression, make it into its own function.

      for (var x = 0; x < str_ary.length; x++) {
        if (WHITE_SPACE_REGEXP.test(str_ary[x]) || str_ary[x] == "") {// Goes on to the next value if there is a space
      ...
      if (has_chords) {
        for (x = 0; x < str_ary.length; x++) {
          if (/^\w/.test(str_ary[x])) {
            str_ary[x] = '<span class="chord" name="' + replace_marks(str_ary[x]) + '">' + replace_marks(str_ary[x]) + '</span>';
          }
        }
      }
...
/******/ });
//# sourceMappingURL=initialize_song-e0254ee028d9c79c6221.js.map
...
/* WEBPACK VAR INJECTION */(function($) {// TODO: Compare with initialize_song and
...
function key_change(old_key, slash_chords) {
  ...
  return function (new_key) {
    console.log(song_body.children);
    ...
    if (print_hidden_tag) {
      print_hidden_tag.value = new_key;
    }
    ...
    if (slash_chords) {
      ...
      var chords_html = song_body.find('span');
...
if (set_list_songs.length == 0) {
  ...
  // set list:
  // get all song bodies, get each value
  // and pass it to key_change for onload
  $('.song_body').each(function (i) {
    var _this = this;
  ...
  for (i = 0; i < song_bodies.length; i++) {
      var keys = song_bodies[i].getAttribute('value').split("/");
      var original_key = keys[0];
      var new_key = keys[1];
       $(() =>
        $('.set_list_song').on(
            'load',
            console.log("key change fire")
           // key_change(original_key, null, new_key, song_bodies[i])
        )
      );
  }
  */
...
/******/ });
//# sourceMappingURL=key_change-d0172bf0cf210506c1c0.js.map